...
public class OkxGridTransService {
    ...
    private final BigDecimal keepBelieveLine = new BigDecimal("0.05");
        ...
        if(list.size() >0){
            ...
            for(DocPredictionOrder o: list) {
                ...
                for(GridOrderView order: gridOrders){
                    ...
                    if("short".equals(order.getDirection()) && p == 1  && probability.compareTo(keepBelieveLine) > 0) {
                        //信号保持
                    }
                }
            }
...
//        if(signal.getProbability().get(signal.getPrediction()).compareTo(new BigDecimal("0.7")) < 0){
//            //没有7成把握不会建立订单
...
//        }
        ...
        //okxGridOrderService.placeOrder(signal.getInstId(),)
        ...
        String maxPx = currentPrice.add(std.multiply(new BigDecimal(0.5))).toString();
        String minPx = currentPrice.subtract(std.multiply(new BigDecimal(0.5))).toString();
        String gridNum = "72";
        String sz = "200";
